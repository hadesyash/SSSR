...
FLAGS = flags.FLAGS
flags.DEFINE_string('normalizers_file', 'normalizers_original.pkl', 'file with pickled feature normalizers')
...
class FeatureNormalizer(object):
    def __init__(self, feature_samples, share_scale=False):
        ...
        # print("Feature Samples Shape:", feature_samples.shape)
        # print("Feature Means Shape:", self.feature_means.shape)
        if share_scale:
    ...
    def normalize(self, sample):
        # print(f"Normalizing: sample shape {sample.shape}, feature_means shape {self.feature_means.shape}")
        if sample.shape[1] != self.feature_means.shape[1]:
            raise ValueError(f"Shape mismatch: sample {sample.shape}, feature_means {self.feature_means.shape}")
