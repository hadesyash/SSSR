...
flags.DEFINE_list('silent_data_directories', ['/home/xiaoyuan/silentspeech/superviseddemo/EMG/gaddycode/silent_speech/emg_data/closed_vocab/silent'], 'silent data locations')
flags.DEFINE_list('voiced_data_directories', ['/home/xiaoyuan/silentspeech/superviseddemo/EMG/gaddycode/silent_speech/emg_data/closed_vocab/voiced'], 'voiced data locations')
flags.DEFINE_string('testset_file', 'testset_largedev_closed_variant_1.json', 'file with testset indices')
...
def load_utterance(base_dir, index, limit_length=False, debug=False, text_align_directory=None):
    ...
    info_path = os.path.join(base_dir, f'{index}_info.json')
    with open(info_path) as f:
        info = json.load(f)
    text = info['text']
    # print(f"Loaded text from: {info_path}, text: {text}")
...
class SizeAwareSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self):
        ...
        for idx in indices:
            # 兼容三元组数据结构
            directory_info, file_idx, _ = self.dataset.example_indices[idx]  # 取前两个值，忽略 sentence_index
...
class EMGDataset(torch.utils.data.Dataset):
    def __init__(self, base_dir=None, limit_length=False, dev=False, test=False, 
                 no_testset=False, no_normalizers=False, only_silent=False, 
                    ...
                    with open(os.path.join(directory_info.directory, fname)) as f:
                        ...
                        if info['sentence_index'] >= 0:
                            ...
                            if (test and location_in_testset and not directory_info.exclude_from_testset) \
                                ...
                                self.example_indices.append((directory_info, int(idx_str), info['sentence_index']))
        ...
        if only_silent:
            self.example_indices = [(dir_info, idx, sent_idx) for dir_info, idx, sent_idx in self.example_indices if dir_info.silent]

        # 随机选择自定义数量的训练样本
        if num_train_samples is not None and not (dev or test):
            random.seed(42)
            self.example_indices = random.sample(self.example_indices, min(num_train_samples, len(self.example_indices)))
    ...
    def __getitem__(self, i):
        directory_info, idx , _ = self.example_indices[i]
